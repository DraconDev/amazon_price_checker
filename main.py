...
def main():
    ...
    def get_price(url):
        driver = webdriver.Chrome("../../_Tools/webdriver/chromedriver.exe")
        driver.get(url)
        price = int(driver.find_element(By.CSS_SELECTOR, "span.a-price-whole").text)
        name = " ".join(driver.find_element(By.ID, "productTitle").text.split(" ")[:3])
        check_deal(price, name, 60)
